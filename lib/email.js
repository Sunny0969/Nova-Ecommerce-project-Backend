...
async function verifyEmailOnStartup() {
  ...
  if (!isEmailConfigured()) {
    console.warn(
      '[email] Order emails DISABLED — set EMAIL_HOST, EMAIL_USER, and EMAIL_PASS (Gmail App Password) in backend/.env'
    );
  ...
  try {
    if (isRailwayHost()) {
      const smtp = buildSmtpOptions();
      console.log(
        `[email] Railway SMTP config — ${smtp.host}:${smtp.port} (verify skipped, sends on orders)`
      );
      return { ok: true, skippedVerify: true };
    }
...
function primaryFrontendUrl() {
  const raw = process.env.FRONTEND_URL || 'https://bazaar-pk.com';
...
async function sendMail({ to, subject, text, html }, attempt = 1) {
  if (!isEmailConfigured()) {
  ...
  try {
    ...
    if (retryable && attempt < 3) {
      console.warn(`[email] Retry ${attempt + 1}/3:`, subject, '→', to);
      await sleep(1500 * attempt);
      return sendMail({ to, subject, text, html }, attempt + 1);
    }
...
function scheduleOrderPlacedEmails(order, user, shippingOverride, res) {
  const payload = {
    order: toPlainDoc(order),
    user: toPlainDoc(user),
    shippingOverride: shippingOverride ? { ...shippingOverride } : undefined
  };

  const run = () => {
    void sendOrderPlacedEmails(payload.order, payload.user, payload.shippingOverride).catch((err) => {
      console.error('[email] Order notification emails failed:', err?.message || err);
    });
  };
