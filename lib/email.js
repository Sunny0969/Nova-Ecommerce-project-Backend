...
function getAdminEmail() {
  return process.env.ADMIN_EMAIL && String(process.env.ADMIN_EMAIL).trim();
}
...
/** Fire order emails in the background so checkout API responds immediately. */
function queueOrderPlacedEmails(order, user, shippingOverride) {
  void sendOrderPlacedEmails(order, user, shippingOverride).catch((err) => {
