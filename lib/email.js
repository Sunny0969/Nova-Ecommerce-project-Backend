...
async function sendOrderStatusUpdateEmail(user, order, previousStatus) {
  ...
  return sendMail({
    ...
    // Unique key per admin update so the customer gets an email on every status change.
    idempotencyKey: `bazaar-order-${id}-status-${previousStatus}-to-${order.status}-${Date.now()}`
  });
}
...
/**
 * Password reset link — valid for 15 minutes (see User.getResetPasswordToken).
 * @param {{ name?: string, email: string }} user
 * @param {string} resetToken — plain token from getResetPasswordToken()
 */
async function sendPasswordResetEmail(user, resetToken) {
  const to = user?.email && String(user.email).trim();
  if (!to) return { skipped: true, reason: 'no_email' };

  const name = user.name && String(user.name).trim() ? String(user.name).trim() : 'there';
  const resetUrl = `${primaryFrontendUrl()}/reset-password/${encodeURIComponent(resetToken)}`;
  const subject = `${STORE_NAME} — Reset your password`;

  const text = [
    `Hi ${name},`,
    '',
    'We received a request to reset your Bazaar account password.',
    '',
...
<html><body style="margin:0;padding:0;background:#f5f5f5;font-family:Arial,sans-serif;color:#111;">
  <div style="max-width:560px;margin:24px auto;background:#fff;border-radius:10px;overflow:hidden;border:1px solid #e8e8e8;">
    <div style="background:#111;color:#fff;padding:20px 24px;">
      <div style="font-size:22px;font-weight:700;">${escapeHtml(STORE_NAME)}</div>
      <div style="margin-top:6px;font-size:15px;opacity:0.9;">Password reset</div>
    </div>
    <div style="padding:24px;">
      <p style="margin:0 0 16px;line-height:1.6;">Hi ${escapeHtml(name)},</p>
      <p style="margin:0 0 20px;line-height:1.65;color:#444;">
        We received a request to reset your account password. Click the button below to choose a new password.
        This link expires in <strong>15 minutes</strong>.
      </p>
      <p style="margin:0 0 24px;">
        <a href="${escapeHtml(resetUrl)}" style="display:inline-block;background:#ea580c;color:#fff;padding:12px 22px;border-radius:6px;text-decoration:none;font-weight:600;">Reset password</a>
      </p>
      <p style="margin:0 0 12px;font-size:13px;color:#666;line-height:1.5;">
        Or copy this link into your browser:<br/>
        <a href="${escapeHtml(resetUrl)}" style="color:#ea580c;word-break:break-all;">${escapeHtml(resetUrl)}</a>
      </p>
  ...
  return sendMail({
    ...
    text,
    html,
    idempotencyKey: `bazaar-pw-reset-${to}-${Date.now()}`
  });
}
