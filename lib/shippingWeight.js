...
/**
 * Flat fallback when weight tiers are off: missing product weight or below legacy threshold.
 */
...
/**
 * Weight-based standard shipping — tier ranges when configured, else legacy threshold + per kg.
 */
function calculateWeightBasedShipping(totalWeightKg, settings) {
  const tiers = normalizeWeightShippingTiers(settings?.weightShippingTiers);
  if (tiers.length) {
