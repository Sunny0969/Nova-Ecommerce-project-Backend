...
function selectStringToProjection(selectStr) {
  const proj = {};
  for (const field of String(selectStr || '').split(/\s+/)) {
    const f = field.trim();
    if (f) proj[f] = 1;
  }
  return proj;
...
/**
 * Admin list query via native driver — avoids Mongoose CastError on legacy category slug $expr filters.
 */
async function adminListProductsUncasted(filter, { select, sort = { createdAt: -1 }, skip = 0, limit = 24 } = {}) {
  const projection = selectStringToProjection(select);
  const coll = Product.collection;
  const [rows, totalCount] = await Promise.all([
    coll.find(filter).project(projection).sort(sort).skip(skip).limit(limit).toArray(),
