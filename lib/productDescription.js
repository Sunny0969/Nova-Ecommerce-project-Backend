...
function getCleanShortDescription(product) {
  ...
  const perfumeShort = normalizeImportedPerfumeShort({ ...product, name });
  if (perfumeShort) return perfumeShort;
...
function stripPerfumeInternalNotes(html) {
  let out = String(html || '');
  if (!out.trim()) return out;

  out = out
    .replace(/<li>\s*<strong>\s*Fulfilment:\s*<\/strong>[\s\S]*?<\/li>\s*/gi, '')
    .replace(/<li>\s*<strong>\s*Wholesale sheet price:\s*<\/strong>[\s\S]*?<\/li>\s*/gi, '')
    .replace(/<li>\s*<strong>\s*Retail price:\s*<\/strong>[\s\S]*?<\/li>\s*/gi, '')
    .replace(/Fulfilment:\s*NOTE\s*:\s*ALL PERFUMES WEIGHT[\s\S]*?(?=<\/li>|\n\n|$)/gi, '')
    .replace(/Wholesale sheet price:\s*\$[\d.]+\s*/gi, '')
    .replace(
      /Retail price:\s*Rs\.\s*[\d,]+\s*\(USD\s*\$[\d.]+\s*[—-]\s*includes\s*\$15\s*service\s*markup\)\s*/gi,
      ''
    )
    .replace(/<ul>\s*<\/ul>/gi, '');

  return stripAdminEditorFooter(out);
}

function hasPerfumeInternalNotes(text) {
  const t = String(text || '');
  return (
    /<strong>\s*Fulfilment:\s*<\/strong>/i.test(t) ||
    /<strong>\s*Wholesale sheet price:\s*<\/strong>/i.test(t) ||
    /<strong>\s*Retail price:\s*<\/strong>.*service markup/i.test(t) ||
    /Fulfilment:\s*NOTE\s*:\s*ALL PERFUMES WEIGHT/i.test(t) ||
    hasAdminEditorFooter(t)
  );
}

function resolveProductDescriptionHtml(product) {
  const raw = product?.description || '';
  if (looksLikeSafeAdminHtml(raw)) return stripPerfumeInternalNotes(raw);
...
function needsDescriptionCleanup(product) {
  ...
  if (hasPerfumeInternalNotes(rawDesc) || hasPerfumeInternalNotes(rawShort)) return true;
  if (hasAdminEditorFooter(rawDesc)) return true;
  if (needsPerfumeShortRewrite(rawShort)) return true;
...
module.exports = {
  ...
  sanitizeProductDoc,
  stripPerfumeInternalNotes,
  hasPerfumeInternalNotes,
