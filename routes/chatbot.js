module.exports = require('./chatbot.impl');

module.exports = require('./chatbot.impl');

(function initChatbotRoute() {
 ...
 * Input: { message, conversationHistory[], sessionId, userId? }
 * Output: { reply, suggestedActions[], escalated }
 */
router.post('/message', async (req, res) => {
  try {
    const client = getOpenAI();
...
async function escalateToHuman({ issue, email, sessionId }) {
  ...
  const session = await ChatSession.findOne({ sessionId: sid }).lean();
  const ticketId = `TCK-${Math.random().toString(36).slice(2, 8).toUpperCase()}-${Date.now().toString(36).slice(-4).toUpperCase()}`;
...
function buildTools({ sessionId, userId }) {
  return [
    {
      type: 'function',
      function: {
...
router.post('/message', async (req, res) => {
  try {
    ...
    const historyFromReq = normalizeHistory(b.conversationHistory);
    const stored = existing?.messages ? existing.messages.slice(-20).map((m) => ({ role: m.role, content: m.content })) : [];
    const convo = (historyFromReq.length ? historyFromReq : stored).slice(-20);
    ...
    const tools = buildTools({ sessionId, userId });
    const ctx = { sessionId, userId };
