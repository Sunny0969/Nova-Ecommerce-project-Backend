/**
 * Replace all pulses products with Imtiaz saved HTML (p1).
 * Adds PRICE_MARKUP Rs to each imported price.
 *
 * Run:
 *   node scripts/importImtiazPulsesFromHtml.js
 */
...
const SOURCE_FILES = [{ path: 'C:/Users/PC/Downloads/p1.htm', section: 'Pulses & Grains' }];
const CATEGORY_SLUG = 'pulses';
const ID_PREFIX = 'imtiaz_pulse';
...
function skuFromImage(url) {
  const s = String(url || '');
  const code = s.match(/\/(CFG\d+|PUL\d+|CON\d+|EDG\d+)\./i);
...
async function main() {
  ...
  console.log(`Removed ${removed.deletedCount} existing pulses products.`);
