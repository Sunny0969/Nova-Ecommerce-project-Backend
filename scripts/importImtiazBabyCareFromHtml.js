/**
 * Replace all baby-care products with Imtiaz saved HTML (b, vb, v, vv).
 * Adds PRICE_MARKUP Rs to each imported price.
 *
 * Run:
 *   node scripts/importImtiazBabyCareFromHtml.js
 */
...
const SOURCE_FILES = [
  { path: 'C:/Users/PC/Downloads/b.htm' },
  { path: 'C:/Users/PC/Downloads/vb.htm' },
  { path: 'C:/Users/PC/Downloads/v.htm' },
  { path: 'C:/Users/PC/Downloads/vv.htm' }
];
const CATEGORY_SLUG = 'baby-care';
const CATEGORY_NAME = 'Baby Care';
const ID_PREFIX = 'imtiaz_baby';
...
function skuFromImage(url) {
  const s = String(url || '');
  const code = s.match(/\/(BBW\d+|DAI\d+|EDG\d+|BEV\d+|CND\d+|CFG\d+|CON\d+)\./i);
