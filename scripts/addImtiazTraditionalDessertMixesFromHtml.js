/**
 * Add traditional-dessert-mixes products from z.htm, zz.htm, zzz.htm.
 * Creates category if missing. Upserts by productId (add mode).
 * Adds PRICE_MARKUP Rs to each imported price.
 *
 * Run:
 *   node scripts/addImtiazTraditionalDessertMixesFromHtml.js
 */
...
const SOURCE_FILES = [
  { path: 'C:/Users/PC/Downloads/z.htm' },
  { path: 'C:/Users/PC/Downloads/zz.htm' },
  { path: 'C:/Users/PC/Downloads/zzz.htm' }
];
const CATEGORY_SLUG = 'traditional-dessert-mixes';
const CATEGORY_NAME = 'Traditional Dessert Mixes';
const ID_PREFIX = 'imtiaz_dessert';
...
function inferBrand(title) {
  ...
  return 'Dessert Mixes';
}

function skuFromImage(url) {
  const s = String(url || '');
  const code = s.match(/\/(EDG\d+|CND\d+|CFG\d+|DAI\d+|CON\d+)\./i);
...
async function ensureCategory() {
  ...
  cat = await Category.create({
    name: CATEGORY_NAME,
    slug: CATEGORY_SLUG,
    description: 'Kheer, kulfa, vermicelli, custard, and jelly mixes',
    displayOrder: 8,
    isActive: true,
    image: {
      url: 'https://images.unsplash.com/photo-1578985545062-69928b1d9587?w=640&auto=format&fit=crop&q=80',
      public_id: ''
    }
  });
...
async function main() {
  ...
  console.log(`Category slug: ${CATEGORY_SLUG} (add mode — existing kept)`);
  ...
  console.log('Done.', { created, updated, fail, added: products.length, categoryTotal: total });
