/**
 * Add pasta-noodles products from q.htm + qq.htm (does not remove existing).
 * Adds PRICE_MARKUP Rs to each imported price.
 *
 * Run:
 *   node scripts/addImtiazPastaNoodlesFromHtml.js
 */
...
const SOURCE_FILES = [
  { path: 'C:/Users/PC/Downloads/q.htm' },
  { path: 'C:/Users/PC/Downloads/qq.htm' }
];
const CATEGORY_SLUG = 'pasta-noodles';
const ID_PREFIX = 'imtiaz_pasta';
...
function extractSection(html) {
  ...
  return m ? m[1].replace(/&amp;/g, '&') : 'Noodles & Pasta';
}

function inferBrand(title) {
  const raw = String(title || '').trim();
  if (!raw) return 'Pasta & Noodles';
...
async function main() {
  ...
  for (let i = 0; i < products.length; i++) {
    ...
    const payload = {
      ...
      stock: 50,
      tags: [brand, row.section, 'Pasta & Noodles'],
      isPublished: true,
