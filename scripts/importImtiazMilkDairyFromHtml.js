/**
 * Replace all milk-dairy products with Imtiaz saved HTML (x, xx, xxx, xxxx).
 * Adds PRICE_MARKUP Rs to each imported price.
 *
 * Run:
 *   node scripts/importImtiazMilkDairyFromHtml.js
 */
...
const SOURCE_FILES = [
  { path: 'C:/Users/PC/Downloads/x.htm' },
  { path: 'C:/Users/PC/Downloads/xx.htm' },
  { path: 'C:/Users/PC/Downloads/xxx.htm' },
  { path: 'C:/Users/PC/Downloads/xxxx.htm' }
];
const CATEGORY_SLUG = 'milk-dairy';
const CATEGORY_NAME = 'Milk & Dairy';
const ID_PREFIX = 'imtiaz_dairy';
...
const BRAND_RULES = [
  ...
  ['Millac', 'Millac'],
  ['Comelle', 'Comelle'],
  ['Dairy King', 'Dairy King'],
  ['Dairy Omung', 'Dairy Omung'],
  ['Good Milk', 'Good Milk'],
  ['Polac', 'Polac'],
  ['NesVita', 'NesVita'],
  ['Milo', 'Milo']
...
function skuFromImage(url) {
  const s = String(url || '');
  const code = s.match(/\/(DAI\d+|BEV\d+|EDG\d+|CND\d+|CFG\d+|CON\d+)\./i);
