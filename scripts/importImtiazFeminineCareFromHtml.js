/**
 * Replace all feminine-care products with Imtiaz saved HTML (s, sss, ssss).
 * Creates category if missing. Adds PRICE_MARKUP Rs to each imported price.
 *
 * Run:
 *   node scripts/importImtiazFeminineCareFromHtml.js
 */
...
const CATEGORY_SLUG = 'feminine-care';
const CATEGORY_NAME = 'Feminine Care';
const ID_PREFIX = 'imtiaz_feminine';
...
function skuFromImage(url) {
  const s = String(url || '');
  const code = s.match(/\/(HBY\d+|BBW\d+|EDG\d+|CND\d+|CFG\d+|CON\d+)\./i);
...
async function ensureCategory() {
  ...
  cat = await Category.create({
    ...
    isActive: true,
    image: {
      url: 'https://images.unsplash.com/photo-1556228720-195a672e8a03?w=640&auto=format&fit=crop&q=80',
      public_id: ''
    }
  });
