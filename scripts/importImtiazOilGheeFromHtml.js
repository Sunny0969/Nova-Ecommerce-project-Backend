/**
 * Replace all oil-ghee products with Imtiaz saved HTML (aaaaa, w, aa, aaa, aaaa).
 * Adds PRICE_MARKUP Rs to each imported price.
 *
 * Run:
 *   node scripts/importImtiazOilGheeFromHtml.js
 */
...
function extractSection(html) {
  ...
  return m ? m[1].replace(/&amp;/g, '&') : 'Oil & Ghee';
}

function inferBrand(title) {
  const raw = String(title || '').trim();
  if (!raw) return 'Oil & Ghee';
...
function skuFromImage(url) {
  const s = String(url || '');
  const code = s.match(/\/(EDG\d+|DAI\d+|CFG\d+|CON\d+)\./i);
...
async function main() {
  ...
  for (let i = 0; i < products.length; i++) {
    ...
    const payload = {
      ...
      stock: 50,
      tags: [brand, row.section, 'Oil & Ghee'],
      isPublished: true,
