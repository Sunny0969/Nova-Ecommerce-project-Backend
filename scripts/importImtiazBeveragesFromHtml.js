/**
 * Replace all beverages products with Imtiaz saved HTML (b1/b2/b3).
 *
 * Run:
 *   node scripts/importImtiazBeveragesFromHtml.js
 */
...
const SOURCE_FILES = [
  { path: 'C:/Users/PC/Downloads/b1.htm', section: 'Malt Drinks' },
  { path: 'C:/Users/PC/Downloads/b2.htm', section: 'Juices & Nectars' },
  { path: 'C:/Users/PC/Downloads/b3.htm', section: 'Water' }
];
const CATEGORY_SLUG = 'beverages';
const ID_PREFIX = 'imtiaz_bev';

const BRAND_RULES = [
  ['Murree Brewery', 'Murree Brewery'],
  ['Nestle', 'Nestle'],
  ['Aquafina', 'Aquafina'],
  ['Dasani', 'Dasani'],
  ['Masafi', 'Masafi'],
...
function skuFromImage(url) {
  const s = String(url || '');
  const bev = s.match(/\/(BEV\d+)\./i);
  if (bev) return bev[1].toUpperCase();
...
async function main() {
  ...
  console.log(`Removed ${removed.deletedCount} existing beverages products.`);
  ...
  for (let i = 0; i < products.length; i++) {
    ...
    const payload = {
      ...
      stock: 50,
      tags: [brand, row.section, 'Beverages'],
      isPublished: true,
